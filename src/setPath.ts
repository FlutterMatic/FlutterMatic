...
const bashrcCommand =
  'echo "export PATH=$HOME/.flutter-sdktest/bin:$PATH" >> ~/.bashrc ; export PATH=$HOME/.flutter-sdktest/bin/:$PATH';
const zshenvCommand =
  'echo "export PATH=$HOME/.flutter-sdktest/bin:$PATH" >> ~/.zshenv ; export PATH=$HOME/.flutter-sdktest/bin/:$PATH';
const bashProfileCommand =
  'echo "PATH=$HOME/.flutter-sdktest/bin/:$PATH" >> ~/.bash_profile ; source $HOME/.bash_profile';
const fishPathCommand =
  'echo "set PATH $HOME/.flutter-sdktest/bin/ $PATH" >> ~/.config/fish/config.fish';
...
export async function setPath(shell: Shell,ec:ExtensionContext): Promise<Output> {
  ...
  try {
    ...
    switch (shellName) {
      ...
      default:
        return {
          error: `Could not set the path for ${shellName}`,
          success: false,
        };
    }
  } catch (e: any) {
